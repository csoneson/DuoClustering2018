#' Apply SC3
#'
#' @importFrom SC3 sc3_prepare sc3_estimate_k sc3
...
apply_SC3 <- function(sce, params, k) {
  ...
  tryCatch({
    ...
    st2 <- system.time({
      ...
      cluster <- as.numeric(SummarizedExperiment::colData(dat)[, paste0("sc3_", k, "_clusters")])
      names(cluster) <- rownames(SummarizedExperiment::colData(dat))
    })
