...
summaries provided with the package. 

# Applying a new clustering algorithm to a provided data set
In order to apply a new clustering algorithm to one of the data sets using the
same framework, you need to generate a function consistent with the `apply_*()`
functions provided in the [GitHub
repository](https://github.com/markrobinsonuzh/scRNAseq_clustering_comparison/tree/master/Rscripts/clustering)
corresponding to the publication. The input arguments to this function should
be:

- a `SingleCellExperiment` object
- a list of parameter values (can be empty, if no parameters are used for the
method)
...
The `df` data frames can then be combined across data sets, filterings and
methods.
